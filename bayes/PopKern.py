...
class Stationary(Kern):
    ...
    def gradients_XX_diag(self, dL_dK_diag, X):
        ...
        assert (dL_dK_diag.size == X.shape[0]) or (
                dL_dK_diag.size == 1), "dL_dK_diag has to be given as row [N] or column vector [Nx1]"
...
class Matern52(Stationary):
    ...
    def Gram_matrix(self, F, F1, F2, F3, lower, upper):
        ...
        def L(x, i):
            return (5 * np.sqrt(5) / self.lengthscale ** 3 * F[i](x) + 15. / self.lengthscale ** 2 * F1[i](
                x) + 3 * np.sqrt(5) / self.lengthscale * F2[i](x) + F3[i](x))
        ...
        orig2 = 3. / 5 * self.lengthscale ** 2 * (
                np.dot(F1lower, F1lower.T) + 1. / 8 * np.dot(Flower, F2lower.T) + 1. / 8 * np.dot(F2lower,
                                                                                                  Flower.T))
